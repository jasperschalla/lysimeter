...
src_path = "/Users/schalla-j/Documents/master/data_preparation/new_pipeline"
src_path_folder = os.path.join(src_path, "processing")
...
def save_days(data, start_index, end_index, date_col, lysi_number):
...
def fetch_ec(data):
    ...
    query = f"""SELECT SUM("P_M_EZ_NEZ") FROM "autogen"."raw" WHERE "Location" = 'fen' AND "Device" = 'pluvio' AND time >= '{start_date}' AND time <= '{end_date}' GROUP BY time(1d) """
    ...
    for point in result.get_points():
        value_lst.append(point["sum"])
        time_lst.append(point["time"])

    client.close()

    df_prec = pd.DataFrame({"Timestamp": time_lst, "precipitation": value_lst})
    ...
    st.session_state["ec_data"] = df_prec
...
with st.sidebar:
    ...
    if type_selector == "gap_check":
        read_path = os.path.join(
            src_path_folder, f"{data_selector}_filled", location_selector
        )
    else:
        read_path = os.path.join(
            src_path_folder, f"{data_selector}_post", location_selector
        )
...
try:
    ...
    if len(datasets) > 0:
        ...
        if type_selector == "gap_check":
                ...
                if st.button("Finish File", type="primary", disabled=(not confirm)):
                    ...
                    for index, dataset in enumerate(datasets):
                        ...
                        if data_selector == "balance":
                            ...
                            pathlib.Path(os.path.join(src_path_folder, "awat")).mkdir(
            ...
            """ ec_data = pd.read_csv(
                os.path.join(src_path_folder, "ec_data", "data.csv"),
                delimiter=";",
            )
            ec_data[ec_data.columns[0]] = pd.to_datetime(ec_data.iloc[:, 0]) """
            ...
            try:
                ...
                if data_selector == "balance":
                    ...
                    with winter_container:
                        ...
                        if st.button("Fetch Pluvio Data", type="primary"):
                            fetch_ec(data)
                        ...
                        if "ec_data" in st.session_state:
                            ...
                            end_index = data.iloc[(data.shape[0] - 1), 0]

                            ec_data_filtered = ec_data[
                                (ec_data[ec_data.columns[0]] >= start_index)
                                & (ec_data[ec_data.columns[0]] <= end_index)
                            ]
                            ...
                            if winter_type_selector == "albedo":
                                ...
                                fig_albedo.add_trace(
                                    go.Scatter(
                                        x=ec_data_filtered[ec_data.columns[0]],
                                        y=ec_data_filtered["Albedo"],
                                        mode="markers+lines",
                                        line=dict(color="red"),
                                        name="Albedo",
                                        showlegend=True,
                                ...
                                if param_selector_post == "Precipitation":
                                    ...
                                    model = sm.OLS(
                                        data[col_selector_post].values.reshape(-1, 1),
                                        sm.add_constant(
                                            pluvio_data.values.reshape(-1, 1)
                                        ),
                                        missing="drop",
                                    )
                                    lm = model.fit()
                                ...
                                if close_gaps:
                                    albedo_fills = ec_data_filtered[
                                        ec_data_filtered["Albedo"] >= albedo_threshold
                                    ].sort_values(by=[ec_data_filtered.columns[0]])
                                ...
                                if close_gaps and winter_type_selector == "albedo":
                                    ...
                                    for name, group in scatter_groups:
                                        fig_pluvio.add_scatter(
                                            ...
                                            y=group[f"{param_selector_post}_filled"],
                                            name="replaced",
                                            mode="markers+lines",
                                            marker=dict(size=2, color="red"),
                            ...
                            if st.button("Fill", type="primary", key="albedo_post"):
                                fill_index = data.index[
                                    data[data.columns[0]].isin(
                                        pluvio_df[pluvio_df.columns[0]]
                                    )
                                ]
                                data.loc[fill_index, col_selector_post] = pluvio_df[
                                    f"{param_selector_post}_filled"
                                ]
                                fill_dates = (
                                    data.loc[fill_index, data.columns[0]]
                                    .dt.date.unique()
                                    .tolist()
                                )
                                ...
                                for date in fill_dates:
                                    ...
                                    dataset_part.to_csv(
                                        os.path.join(
                                            st.session_state["read_path"],
                                            f"{lysimeter_selector_post}_{file_date_str}.csv",
                                        ),
                                        index=False,
                                    )
                                # st.rerun()
